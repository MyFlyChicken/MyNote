...
| 仲裁段 | 通过线与，节点**数值小的将停止发送**，由发送方停止![image-20240717152851327](./assets/image-20240717152851327.png) |
| 控制帧 |                                                              |
| 数据帧 | ![image-20240717161503207](./assets/image-20240717161503207.png) |
| CRC    | CRC计算：起始+仲裁+控制+数据<br />CAN2.0 15bit<br />CANFD 17 or 21bit |
| ACK    | 包含ACK 槽(ACK Slot)和 ACK 界定符 2 个位构成<br />接收方校验信息正确后，会将ACK Slot 置为显性位 |
| 结束帧 | 连续7位隐性位，IFS为帧间隔空间，区分不同的帧                 |
...
遥控帧是接收单元向具有相同ID的发送单元请求数据的帧。CANFD没有遥控帧，遥控帧相对于数据帧只是没有数据位
